        ...
        {settings.map(setting => (
          <div key={setting.key_name} className="border-b border-gray-100 last:border-0 pb-6 last:pb-0">
            ...
            <div className="flex mt-1">
              ...
              <button 
                onClick={() => {
                  const val = (document.getElementById(`input-${setting.key_name}`) as HTMLInputElement).value;
                  handleUpdate(setting.key_name, val);
                }} 
                className="bg-blue-600 text-white px-6 py-3 rounded-r-md hover:bg-blue-700 flex items-center transition-colors font-medium"
              >
            ...
            <p className="text-sm text-gray-500 mt-2">
              {setting.key_name === 'whatsapp_number' ? "Ce numéro s'affichera sur la page de demande de devis." : "Cette adresse recevra les notifications."}
            </p>
          </div>
        ))}
      </div>
